...
// props나 state의 경우에는 tagName처럼 명확하게 string이라는 걸 모를 수도 있기 때문에 인덱싱 가능한 타입으로 [key: ]를 작성해주고, 어떤 값이 들어올지는 모르니까 unknown으로 써준다.

export class Component {
  public el;
  public props;
  public state; // 이렇게 각각 앞에 public을 붙여주어 해당 `Component`라는 클래스 안에서 전역적으로 사용할 수 있도록 함.
...
export class Store<S> {
  // <S> : 생성자 함수로 생성이 될 때 Generic 타입으로 정의, about.ts 파일에서 정의된 `State` interface 타입 그대로 받아오게 됨.
